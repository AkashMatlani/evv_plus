...
class UpcomingScuduleScreenOne  extends StatefulWidget {
@override
_UpcomingScuduleScreenOneState createState() => _UpcomingScuduleScreenOneState();
}

class _UpcomingScuduleScreenOneState extends State<UpcomingScuduleScreenOne> {
  ...
  void _getPastDueList(String nurseId, BuildContext context) {
    Utils.showLoader(true, context);
    _scheduleViewModel.getUpCommingListAPICall(nurseId, (isSuccess, response) {
      Utils.showLoader(false, context);
      isLoading = false;
      if (isSuccess) {
        setState(() {
          ...
          _filterList = _pastVisitList;
          Utils.upcommingCountCount =_pastVisitList.length;
        });
      } else {
        setState(() {
          _pastVisitList = [];
